...
describe('WishlistsController', () => {
  ...
  describe('getUserWishlists', () => {
    it('should return user wishlists', async () => {
      const wishlists = await controller.getUserWishlists({ id: 123 } as User);
      expect(wishlists).toContainEqual(testWishlist);
    });

    it("shouldn't return other user wishlists", async () => {
      const wishlists = await controller.getUserWishlists({
        id: 12345,
      } as User);
  ...
  describe('createWishlist', () => {
    it('should create wishlist', async () => {
      ...
      const wishlist = await controller.createWishlist(
        { id: 123 } as User,
        createData,
      );
    ...
    it('should return error if product not found', async () => {
      ...
      await expect(
        controller.createWishlist({ id: 123 } as User, createData),
  ...
  describe('updateWishlist', () => {
    it('should update wishlist', async () => {
      ...
      const { id } = await controller.createWishlist(
        { id: 123 } as User,
        createData,
      );
      const updateData = generate(WishlistCreateDto, true);
      updateData.productIds = [testProduct.id];
      const wishlist = await controller.updateWishlist(
        { id: 123 } as User,
        id,
        updateData,
      );
    ...
    it('should return error if product not found', async () => {
      ...
      await expect(
        controller.updateWishlist({ id: 123 } as User, id, updateData),
      ).rejects.toThrow(NotFoundError);
    });
    ...
    it('should return error if wishlist not found', async () => {
      const updateData = generate(WishlistCreateDto, true);
      await expect(
        controller.updateWishlist({ id: 123 } as User, 12345, updateData),
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('deleteWishlist', () => {
    it('should delete wishlist', async () => {
      ...
      await controller.deleteWishlist({ id: 123 } as User, id);
      expect(
    ...
    it('should return error if wishlist not found', async () => {
      await expect(
        controller.deleteWishlist({ id: 123 } as User, 12345),
      ).rejects.toThrow(NotFoundError);
    });
  });
});
